...
/*引脚配置*/
#define OLED_W_SCL(x)		GPIO_WriteBit(GPIOB, GPIO_Pin_8, (BitAction)(x))//SCL接的位置
#define OLED_W_SDA(x)		GPIO_WriteBit(GPIOB, GPIO_Pin_9, (BitAction)(x))//SDA接的位置
...
//    // 设置页地址
//    OLED_WriteCommand(0xB0 + (y / 8)); // 页地址，y / 8 计算页号
//    // 设置列地址高字节
//    OLED_WriteCommand(((x & 0xF0) >> 4) | 0x10); // 高4位
//    // 设置列地址低字节
//    OLED_WriteCommand((x & 0x0F) | 0x00); // 低4位
//}
